...
exports.onGGWritten = (0, firestore_1.onDocumentWritten)("ggs/{ggId}", async (event) => {
    ...
    if (creatorSnap.exists) {
        ...
        if (after && creatorData.fcmToken && creatorId !== after.userId) {
            ...
            await sendPushNotif(creatorData.fcmToken, '⭐ GG received!', `${senderName} gave you a GG on your clip!`, { screen: 'Feed', videoId });
        }
    }
    v2_1.logger.info(`onGGWritten: vidéo ${videoId} → ggCount=${realGGCount}, créateur ${creatorId} → ggReceived=${totalGGReceived}`);
});
// ─────────────────────────────────────────────────────────────────────────────
...
exports.onVideoDeleted = (0, firestore_1.onDocumentDeleted)("videos/{videoId}", async (event) => {
    ...
    v2_1.logger.info(`onVideoDeleted: vidéo ${videoId} — ${ggsSnap.size} GGs nettoyés, créateur ${creatorId} ggReceived=${totalGGReceived}`);
});
// ─────────────────────────────────────────────────────────────────────────────
